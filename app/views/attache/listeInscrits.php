   ...
   <div class="bg-white  shadow p-4 flex justify-between items-center rounded-bl-3xl rounded-tr-3xl m-6">
       ...
       <form id="filterForm" method="get" class="flex items-center space-x-4">
           ...
           <input type="hidden" name="page" value="listeInscrits">
           <select name="classe" onchange="document.getElementById('filterForm').submit()"
               ...
               <?php foreach ($classe as $val): ?>
                   <option value="<?= $val['id'] ?>" <?= (isset($_GET['classe']) && $_GET['classe'] == $val['id']) ? 'selected' : '' ?>><?= $val['libelle'] ?></option>
               <?php endforeach ?>
           </select>
       </form>
       ...
       <button type="submit"
           ...
           <a href="<?= PAGE ?>controller=inscrit&page=formInscrit">
               <i class="ri-user-add-line"></i>
               <span>Inscription</span></a></button>
   </div>
   <!-- Liste des classes en cards -->
   <?php if ($inscrit != null): ?>
       <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
           <?php foreach ($inscrit as $value): ?>
               <div class="bg-white border rounded-xl flex-col shadow-md p-4 hover:shadow-lg transition">
                   <div class="flex justify-between">
                       ...
                       <div class="flex gap-1">
                           <a href="<?= PAGE ?>?controller=inscrit&page=formInscrit&edit=<?= $value["id"] ?>" class="text-gray-700 hover:text-pink-600 rounded-lg ">
                               <i class="ri-pencil-line text-medium"></i>
                           </a>
                           <a href="<?= PAGE ?>?controller=inscrit&page=annulerInsrit&delete=<?= $value["id"] ?>"
                   ...
                   <button class="w-full ml-24">
                       <a href="<?= PAGE ?>?controller=inscrit&page=formInscrit&reinscrit=<?= $value["id"] ?>"
   ...
   <div class="flex justify-center mt-6">
       <nav class="inline-flex rounded-md shadow-sm isolate" aria-label="Pagination">
           ...
           <a href="#" class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-pink-600 border border-gray-300 hover:bg-red-700">1</a>
